...
class BiLSTMCRF:
    ...
    def _build_model(self):
        ...
        bilstm_layer = keras.layers.Bidirectional(
            keras.layers.LSTM(units=self.lstm_units,
                              recurrent_dropout=0.4,
                              return_sequences=True),
            name='Bi-LSTM',
        )(embedding_layer)
        dense_layer = keras.layers.TimeDistributed(
            Dense(units=self.dense_units, activation=K.relu),
            name='td_dense',
        )(bilstm_layer)
    ...
    def _convert_label_seqs_to_idx(self, label_seqs: List[List[str]]) -> List[List[int]]:
        """label映射index"""
        return [[self.label2idx[label] for label in label_seq]
                for label_seq in label_seqs]

    def _convert_idx_seqs_to_label(self,
                                   idx_seqs: List[List[int]],
                                   raw_lens: List[int]) -> List[List[str]]:
        """index映射类别"""
        label_seqs = []
        for idx_seq, raw_len in zip(idx_seqs, raw_lens):
            indices = idx_seq[:raw_len]
            label_seqs.append([self.idx2label[i] for i in indices])
        return label_seqs
    ...
    def _data_generator(self,
        ...
        while True:
            ...
            for i in range(steps):
                ...
                tokenized_x = self._tokenize(batch_x)
                idx_y = self._convert_label_seqs_to_idx(batch_y)

                padded_x = sequence.pad_sequences(
                    tokenized_x, maxlen=self.max_seq_len,
                    padding='post', truncating='post',
                    value=self.token2idx[H.PAD],
                )
                padded_y = sequence.pad_sequences(
                    idx_y, maxlen=self.max_seq_len,
                    padding='post', truncating='post',
    ...
    def fit(self,
        ...
        if X_val:
            ...
            fit_kwargs['validation_steps'] = math.ceil(len(X_val) / batch_size)
    ...
    def save_dict(self, dict_root_path):
        with open(os.path.join(dict_root_path, 'vocab.json'), 'w', encoding='utf-8') as f:
            json.dump(self.token2idx, f, indent=2, ensure_ascii=False)
        with open(os.path.join(dict_root_path, 'labels.json'), 'w', encoding='utf-8') as f:
            json.dump(self.label2idx, f, indent=2, ensure_ascii=False)
    ...
    def load_model(cls, model_path, dict_root_path):
        ...
        agent.model.summary()
        with open(os.path.join(dict_root_path, 'vocab.json'), 'r', encoding='utf-8') as f:
            agent.token2idx = json.load(f)
        with open(os.path.join(dict_root_path, 'labels.json'), 'r', encoding='utf-8') as f:
            agent.label2idx = json.load(f)
