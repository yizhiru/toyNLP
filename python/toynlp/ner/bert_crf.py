...
class BertCRF:
    ...
    def _load_bert_model(self):
        ...
        logging.info(f'loading bert model from {self.bert_model_path}')
        return keras_bert.load_trained_model_from_checkpoint(
    ...
    def _build_model(self):
        ...
        dense_layer = TimeDistributed(
            Dense(self.dense_units, activation=K.tanh),
            name='td_dense',
        )(bert_model.output)
    ...
    def _convert_label_seqs_to_idx(self, label_seqs: List[List[str]]) -> List[List[int]]:
        def convert_label_seq(label_seq: List[str]):
            idx_seq = [self.label2idx[label] for label in label_seq]
    ...
    def _convert_idx_seqs_to_label(self,
        ...
        for idx_seq, raw_len in zip(idx_seqs, raw_len_seqs):
            indices = idx_seq[1: min(raw_len, self.sequence_len - 2) + 1]
    ...
    def predict(self,
        ...
        padded_tokens = sequence.pad_sequences(
            tokens, maxlen=self.sequence_len,
            padding='post', truncating='post',
            value=self.token2idx[H.PAD],
        )
    ...
    def save_dict(self, dict_path):
        with open(os.path.join(dict_path, 'labels.json'), 'w', encoding='utf-8') as f:
    ...
    def load_model(cls, model_path, bert_model_path, dict_path, sequence_len):
        ...
        with open(os.path.join(dict_path, 'labels.json'), 'r', encoding='utf-8') as f:
