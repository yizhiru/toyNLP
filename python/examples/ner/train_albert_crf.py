...
from toynlp.ner import AlbertCRF  # noqa: E402
...
def parse_args():
    parser = argparse.ArgumentParser(description='ALBERT-CRF NER 训练与评估')
    parser.add_argument('--seq-len', type=int, default=128)
    parser.add_argument('--epochs', type=int, default=50)
    parser.add_argument('--batch-size', type=int, default=64)
    parser.add_argument('--device-map', type=str, default='3')
    parser.add_argument('--albert-model-path', type=str, default='albert_tiny_489k')
...
def main():
    ...
    model = AlbertCRF().load_model(
        model_path=os.path.join(args.output_path, 'ner.h5'),
        bert_model_path=args.albert_model_path,
        dict_path=args.output_path,
        sequence_len=args.seq_len,
    )
